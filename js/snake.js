class SnakeGame {
  constructor() {
    this.$application = document.querySelector('#wrapper');
    this.$canvas = this.$application.querySelector('canvas');
    this.ctx = this.$canvas.getContext('2d');
    this.$popupscreen = this.$application.querySelector('.popup-screen');
    this.$score = this.$application.querySelector('.score');
    ...
    this.soundEffects = {
      score: new Audio('./sounds/eat.mp3'),
  ...
  init() {
    ...
    this.$popupscreen.querySelector('.settings').addEventListener('click', event => {
      this.chooseDifficulty(event.target.dataset.difficulty);
    });

    // Play
    this.$popupscreen.querySelector('.playbtn').addEventListener('click', () => {
      this.startGame();
    });
  }
  ...
  chooseDifficulty(difficulty) {
    if(difficulty) {
      ...
      this.$popupscreen.querySelectorAll('.settings button').forEach(btn => btn.classList.remove('active'));
  ...
  startGame() {
    ...
    this.$application.classList.add('game-in-progress');
    this.$application.classList.remove('game-over');
  ...
  endGame() {
    ...
    this.$application.classList.remove('game-in-progress');
    this.$application.classList.add('game-over');
    this.$popupscreen.querySelector('.settings h3').innerText = 'Game Over';
    this.$popupscreen.querySelector('.settings .end-score').innerText = `Score: ${this.game.score}`;
